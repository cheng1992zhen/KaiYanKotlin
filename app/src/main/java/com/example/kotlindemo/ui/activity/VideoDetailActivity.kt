...
import com.example.kotlindemo.util.WatchHistoryUtils
import com.example.kotlindemo.util.showToast
import com.example.kotlindemo.view.VideoListener
...
class VideoDetailActivity : BaseActivity(), VideoDetailContract.View {
    ...
    private fun saveWatchVideoHistoryInfo(watchItem: HomeBean.Issue.Item) {
        ...
        for ((key, _) in historyMap) {
            if (watchItem == WatchHistoryUtils.getObject(Constants.FILE_WATCH_HISTORY_NAME,App.context, key as String)) {
                WatchHistoryUtils.remove(Constants.FILE_WATCH_HISTORY_NAME,App.context, key)
            }
        }
        WatchHistoryUtils.putObject(Constants.FILE_WATCH_HISTORY_NAME,App.context, watchItem,"" + mFormat.format(
            Date()
        ))
    }
